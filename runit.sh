...
# After WSI and definition is complete, the script will put out two files in output: an senseval2 xml file with 
# each context assigned to a cluster, and a *.def file, which has a definition for each each cluster
# The *.def file 
# In addition the scripts will run sensecluster_scorer on the clusters generated and print the output
...
#echo "Removing stopwords, and converting $1 to hdp-wsi format"
